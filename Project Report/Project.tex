...
This matches up with figure~\ref{point-plot} where a noticeable gap between the average score and the model scores appears during those gameweeks, driven by chip usage. During almost all other weeks both models outperformed the average manager. With more data and a full-season simulation, the models would likely show a clearer performance advantage over the average score.

Focusing back on the prediction models, the final model trained with Twitter data outperforms the model trained without by around 25 points over the course of the 8 gameweek period. This could suggest a positive impact from including Twitter data, but again the time period for evaluation is not long enough to draw a solid conclusion. Zooming in on some of the point differences between the two models, gameweeks 32 and 34 stand out to be the largest gaps. Exploring the teams selected by each model during those weeks provides some more insights:

\begin{table}[ht]
\centering
\caption{Common Players Selected by Both Models (GW32)}
\begin{tabular}{lcc}
\toprule
\textbf{Player} & \textbf{Expected Points (xp)} & \textbf{Actual Points} \\
\midrule
Trent Alexander-Arnold & 5.2 / 5.1 & 5 \\
Kieran Trippier         & 5.2 / 5 & 2 \\
Virgil van Dijk         & 4.7 / 4.6 & 1 \\
Gabriel Martinelli      & 5.5 / 5.4 & 9 \\
Martin Ødegaard         & 5.2 / 5.2 & 7 \\
James Maddison          & 4.9 / 5 & 0 \\
Ivan Toney              & 4.7 / 4.9 & 9 \\
Eberechi Eze            & 4.8 / 4.7 & 3 \\
\bottomrule
\end{tabular}
\end{table}
...
The players selected exclusively by the final model outscored those chosen exclusively by the non-sentiment model by 15 points thanks to the 7 and 13 point hauls from Mohammed Salah and Alexander Isak. 
% -------------------------------------------------------------
