...
Firstly, the only text value feature in the dataset, the team name, was encoded using one-hot encoding. This is where a new column is introduced for every possible value the feature could take, so new columns for all 20 Premier League teams were introduced. Then for example, if the original row in the dataset had a team value of 'Manchester United', the corresponding 'manchester\_united' column would have a value of 1, and all other team columns would have a value of 0. 
...
After this any rows with empty values or duplicate rows were dropped from the dataset completely, resulting in a dataset ready to be used for model training.
