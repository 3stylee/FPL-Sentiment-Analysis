...
Linear Programming is a mathematical optimisation technique used to solve problems where the objective is to maximise/minimise a linear function, subject to a set of linear constraints \cite{lp}. The linear function to be maximised/minimised is made up of \textbf{decision variables} which represent the choices to be made in the problem. For the example of selecting a fantasy football team to maximise points scored, each player can have a binary decision variable (0 or 1) representing whether they should be chosen or not. The linear function to be maximised would be:
\[
...
This example belongs to a more specific category of linear programming known as Linear Integer Programming (LIP) where some or all decision variables must be integers. These types of problems exist in many real-world examples, where for example the number of units of a product to produce to optimise stock must be an integer.

Linear constraints are also placed on the problem, usually making it non-trivial to solve and thus more advanced techniques must be used to solve these problems.

\subsection*{The PuLP Library}
The PuLP \cite{pulp} library is a linear and mixed integer programming library that provides a solver class to solve linear programming problems. It does this by using a technique known as branch and bound, which is commonly used for solving LIP problems.

Branch and bound works by repeatedly
% -------------------------------------------------------------
% METHODOLOGY
% ------------------------------------------------------------- 
...
This study uses fine-tuning to refine BERT for the specific task of sentiment analysis on FPL posts. Fine tuning is the process of adapting a pre-trained model for specific tasks or use-cases \cite{fine-tuning}. This is a much less computationally demanding task than fully training a model, requiring only a relatively small dataset - ideal for a project of this scale. Combining the extensive natural language knowledge baked into BERT with a suitable dataset of FPL posts, should result in a sophisticated and accurate sentiment analysis model that suits the needs of the project.
...
Detailed in section~\ref{section:team-selection}, the final model was evaluated on unseen data over the course of an 8 gameweek period, in order to evaluate how it performed under real-world conditions. The results of simulating model performance for the final 8 gameweeks of the season were plotted (\ref{point-plot}, and \ref{cumulative-point-plot}). The final model was compared alongside the best possible score, the average score for all players, and the model trained without X data.
...
Focusing back on the prediction models, the final model trained with X data outperformed the model trained without by around 25 points over the course of the 8 gameweek period. This could suggest a positive impact from including X data, however the time period for evaluation is not long enough to draw a solid conclusion. Zooming in on some of the point differences between the two models, gameweeks 32 and 34 stand out to be the largest gaps. Exploring the teams selected by each model during these weeks provides some more insights:
...
While Mohammed Salah had a large number of positive posts (19), which may have contributed to his selection, both Nick Pope and Bruno Guimarāes had a similarly high number of positive posts (14 and 17) but were not chosen by the final model. The highest scorer in Alexander Isak actually had the same amount of negative posts as positive (6) so it seems unlikely his selection by the final model was influenced by the X features.

A similar analysis was conducted for gameweek 34, which had similar results. Whilst there were differences in player selection favouring the final model, post sentiment alone did not consistently explain these choices. This evidence seems to indicate the observed performance differences between the final model and the model trained with no X information are likely not due to the inclusion of X sentiment data alone.
% -------------------------------------------------------------
