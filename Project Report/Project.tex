...
This chapter outlines the approaches taken towards developing the components that make up the final prediction model. The choices taken for the specific sentiment analysis and expected points models are outlined, including the model selection, data identification and processing, training and hyper-parameter tuning, as well as evaluation methods. The team selection algorithm is explained, and the manner in which these components combine to form the final model are detailed.
% -------------------------------------------------------------
% OVERVIEW OF THE APPROACH
% -------------------------------------------------------------
\section[Overview of the Approach]{Overview of the Approach}
This project focuses on developing and evaluating a machine learning model for predicting FPL player performance using sentiment analysis and historical data. The study consists of three main stages: sentiment analysis, expected points prediction, and performance evaluation.

The first stage involves fine-tuning a BERT-based sentiment analysis model to classify tweets about FPL players before each gameweek. The aggregated sentiment scores are then integrated into a dataset containing historical player performance data. While existing FPL expected points models rely solely on structured historical data, this approach investigates whether incorporating Twitter sentiment can improve predictive performance.

In the second stage, a gradient boosting machine (GBM) model is trained on the new dataset to estimate expected points for each player. The model’s output represents its prediction of how many FPL points a player is likely to score in the upcoming gameweek.

Finally, the model's effectiveness is assessed by applying an algorithm that simulates FPL team selection. The team selection process is formulated as an optimisation problem, where integer linear programming is used to construct a 15-player squad that maximises expected points while adhering to budget and constraint rules. This evaluation aims to quantify the impact of sentiment analysis on decision-making in FPL over the course of a whole season.

By incorporating Twitter data into player performance predictions, this research explores the extent to which social media data can enhance existing FPL forecasting models. The findings may provide valuable insights for FPL managers seeking data-driven strategies to improve their decision-making.
% -------------------------------------------------------------
...
Additionally, the constraints mentioned above are modelled as a linear set of rules for the optimiser to follow when solving the problem. The PuLP library is utilised to explore all possible solutions in an effective way using branch-and-bound techniques. If an optimal solution is possible, the solver will output it after finishing the search through the solution space.
...
With the team selection algorithm in place, a set of teams for each gameweek (31-38) and each model (all data, no sentiment data, and true values) was created. 
...
% IMPLEMENTATION
% -------------------------------------------------------------
This chapter describes the detailed implementation of the points prediction model, detailing the libraries, APIs, and methodologies used for each component. It outlines the machine learning algorithms and training pipelines, with each stage of the pipeline such as data collection, pre-processing, model training, and hyper-parameter tuning systematically explained. 

Furthermore, details of the metrics used to assess model performance are discussed, along with the strategies used to optimise these metrics. Key considerations made during training, such as tradeoffs between bias and variance are also explored to ensure the final model is robust and can generalise well.
% -------------------------------------------------------------
% SENTIMENT ANALYSIS MODEL
% -------------------------------------------------------------
\section[Sentiment Analysis Model]{Sentiment Analysis Model}
...
Model outputs appear to be mostly correct and are easily retrieved through the built-in methods provided by the transformers library. This allows for a quick and efficient way to generate a list of sentiment predictions for a large set of tweets, necessary for building the final model. 
...
After this was done duplicates were dropped and the dataset was examined to ensure the correct columns existed, with appropriate values. Finally the dataset was split into test, train and final evaluation datasets. The purpose of the final evaluation dataset was to keep hidden data, so the final model could be fairly evaluated without any data leakage during training. The test and train set were used as normal for model training, with around 15k training samples and 3k test samples. 
