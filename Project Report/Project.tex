...
The primary goal of this project is to evaluate how incorporating sentiment data taken from Twitter affects model performance compared to models relying solely on historical features. As such it made sense to train another model using the same dataset, with the exception of the \texttt{'positive\_tweets'} and \texttt{'negative\_tweets'} features. 

This set up allows for a fair comparison between the two models, and any notable differences could be observed. The first comparison was the RMSE of validation data between the two models. The model with sentiment features produced a RMSE of 0.0723, while the model trained without sentiment features scored 0.0725. This small difference favours the model trained with sentiment features, however it is hard to draw conclusions from such a small change.

To further explore this, matplotlib was used to visualise the distribution of the residuals (the difference between true and predicted values) for each model. 
	...
	\caption{Residuals distribution for each model.}
...
As shown in Figure~\ref{residuals-distribution}, the sentiment-based model had a higher peak around zero, indicating more predictions that were exactly correct or only slightly incorrect. In contrast, the model without sentiment data has slightly more spread out predictions which implies its predictions tend to deviate further from the true values. 

This supports the idea that the incorporation of sentiment data helps the model to gather more information and make more accurate predictions, reducing any large errors. It seems that positive sentiment can act as a proxy for additional crowd wisdom. It is also worth noting the most important feature 'transfers\_in\_last\_week' (how many people bought a player) is another feature that reflects collective manager behaviour, reinforcing the idea that aggregating public sentiment and behaviour provides value.
...
As further evidence, the feature importance plot (Figure~\ref{feature-importance}) ranks 'positive\_tweets' in 10th place, just behind 'points\_last\_week', suggesting that positive sentiment contributes meaningfully to the model. Notably, negative sentiment did not appear among the top features, indicating that positive sentiment may have a stronger correlation with future performance than negative sentiment.
% -------------------------------------------------------------
...
Then for each gameweek and model, the \texttt{select\_fpl\_team} function defined earlier was called with the relevant data to get the optimal squad. This was run through the \texttt{select\_starting\_xi} function to get the best eleven players, and then the actual points scored by that eleven was calculated using \texttt{.sum()}. The best player (player with highest expected points) from the eleven had their points added to the total again to simulate their points being doubled (captaincy).
